...
func _on_http_request_completed(result, response_code, headers, body):
	...
	print("🔁 Contenu brut:", response_text)

	# Essayer de parser la réponse JSON même si code != 200
	var response = JSON.parse_string(response_text)

	if response == null:
		print("❌ Erreur JSON ou réponse invalide :", response_text)
		return

	if response_code != 200:
		# Affiche le message exact retourné par le backend
		print("❌ Erreur serveur :", response.get("message", "Erreur inconnue"))
		return
